...
const packageValue = [
  { key: "คอนโด" },
  { key: "บ้านเดี่ยว/ทาวน์โฮม/ออฟฟิศสำนักงาน (ขนาดพื้นที่ไม่เกิน 500 ตร.ม.)" },
];

const FormSprayPage = ({ userId }) => {
  ...
  return success ? (
    <FormSuccessPage headingText="ฉีดพ่นฆ่าเชื้อ" />
  ) : (
    <ContainerElement>
      <FormWrapperElement>
        <FormHeaderElement headingText="ฉีดพ่นฆ่าเชื้อ" />
...
const addData = async (formData) => {
  try {
    const docRef = await addDoc(collection(db, "spray"), {
      id: formData.id,
